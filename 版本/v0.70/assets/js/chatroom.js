$(document).ready(function(){
    ...
    const memberListTeam = $("#member-list-team");
    const memberListOnline = $("#member-list-online");
    const memberListOffline = $("#member-list-offline");
    var memberObj = {
        id:0,
        username:"姓名",
        permissions:"student",
    ...
    //     $msg = CreatMember("姓名",Math.floor(Math.random()*30),"online");
    //     memberListOnline.scrollTop(chatroomCon[0].scrollHeight);

    // }

    //創建用戶，inputstate分為online,offline,team，會置入不同的清單上
    function CreatMember(inputName = "姓名",inputLV = 0,inputState = "online"){

        memberObj["username"] = inputName;
        memberObj["lv"] = inputLV;
        ...
        if(inputState = "online"){
            memberListOnline.append($msg);
            memberListOnline.scrollTop(chatroomCon[0].scrollHeight);
        }
        else if(inputState = "offline"){
            memberListOffline.append($msg);
            memberListOffline.scrollTop(chatroomCon[0].scrollHeight);
        }
        else if(inputState = "team"){
            memberListTeam.append($msg);
            memberListTeam.scrollTop(chatroomCon[0].scrollHeight);
        }
    ...
    //改變某成員的列表，未完成
    function ChangeMemberClass(inputMemberName,inputState = "online"){
        if(inputState = "online"){
            memberListOnline.append($msg);
            memberListOnline.scrollTop(chatroomCon[0].scrollHeight);
        }
    ...
    //改變成員列表人數
    function ChangeMemberListPeopleNumber(inputState = "online",inputNewPeopleNum){
        if(inputState = "online"){
            memberListOnline.find(".member-message").text(`組員 ─ ${inputNewPeopleNum}人`);
        }
        else if(inputState = "offline"){
            memberListOffline.find(".member-message").text(`上線中 ─ ${inputNewPeopleNum}人`);
        }
        else if(inputState = "team"){
            memberListTeam.find(".member-message").text(`離線中 ─ ${inputNewPeopleNum}人`);
        }
    }
